...
class LiftedStructureLoss(GenericPairLoss):
    def __init__(self, neg_margin=1, pos_margin=0, margin_activation="relu",  **kwargs):
        """
    ...
    def _compute_loss(self, pos_pairs, neg_pairs, indices_tuple):
        ...
        if len(a1) > 0 and len(a2) > 0:
            ...
            if self.margin_activation == "soft_plus":
                loss_per_pos_pair = torch.nn.functional.softplus(loss_per_pos_pair)
            elif self.margin_activation == "relu":
                loss_per_pos_pair = torch.nn.functional.relu(loss_per_pos_pair)
            elif self.margin_activation == "leaky_relu":
                loss_per_pos_pair = torch.nn.functional.leaky_relu(loss_per_pos_pair)
            elif self.margin_activation == "hard_swish":
                loss_per_pos_pair = torch.nn.functional.hardswish(loss_per_pos_pair)
            elif self.margin_activation == "selu":
                loss_per_pos_pair = torch.nn.functional.selu(loss_per_pos_pair)
            elif self.margin_activation == "celu":
                loss_per_pos_pair = torch.nn.functional.celu(loss_per_pos_pair)
            elif self.margin_activation == "gelu":
                loss_per_pos_pair = torch.nn.functional.gelu(loss_per_pos_pair)
